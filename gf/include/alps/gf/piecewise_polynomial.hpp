 ...
 */
        ...
        private:
            ...
            void set_validity() {
                ...
                assert(valid_);
                valid_ = valid_ && ((ptrdiff_t)section_edges_.size() == n_sections_ + 1);
                assert(valid_);
                valid_ = valid_ && ((ptrdiff_t)(coeff_.shape()[0]) == n_sections_);
                assert(valid_);
                valid_ = valid_ && ((ptrdiff_t)(coeff_.shape()[1]) == k_ + 1);
                assert(valid_);
        ...
        public:
            ...
            inline T& coefficient(int i, int p) {
                assert(i >= 0 && i < (int)section_edges_.size());
                assert(p >= 0 && p <= k_);
#ifndef NDEBUG
                check_validity();
#endif
            ...
            T overlap(const piecewise_polynomial<T2> &other) const {
                ...
                for (int s = 0; s < n_sections_; ++s) {
                    ...
                    for (size_t p = 1; p < dx_power.size(); ++p) {
